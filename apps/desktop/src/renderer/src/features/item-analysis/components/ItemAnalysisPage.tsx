...
import { useItemAnalysisData } from "../hooks/useItemAnalysisData";
...
export default function ItemAnalysisPage() {
  ...
  const { result, loading, error, analyze } = useItemAnalysisData();
          ...
          result.characters.length === 0 ? (
            ...
            <ResultsWrapper>
              {result.characters.map((char) => (
                <CharacterSection key={char.characterNum} data={char} />
              ))}
            </ResultsWrapper>
          )
        )}
