...
import { usePhaseCombatData } from "../hooks/usePhaseCombatData";
import { PhaseSummarySection } from "./PhaseSummarySection";
import { SurvivalDistributionSection } from "./SurvivalDistributionSection";
import { CharacterMatchupSection } from "./CharacterMatchupSection";
import { DeathAreaSection } from "./DeathAreaSection";
...
export default function PhaseCombatPage() {
  const [nickname, setNickname] = useState("");
  const { result, loading, error, analyze } = usePhaseCombatData();
  ...
  return (
    <PageWrapper>
      ...
      <PageContent>
        <SectionTitle>
          <Text variant="h2">전투 스타일 분석</Text>
        </SectionTitle>
        ...
        {!loading && result && (
          result.gamesAnalyzed === 0 ? (
            ...
            <ResultStack>
              <PhaseSummarySection data={result} />
              <SurvivalDistributionSection games={result.games} />
              <DeathAreaSection deathAreaStats={result.deathAreaStats} />
              <CharacterMatchupSection data={result} />
            </ResultStack>
          )
        )}
      </PageContent>
