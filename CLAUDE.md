...
--   damage_to_player, damage_from_player, play_time }
-- ※ damage_* / play_time 은 재수집 이후 데이터부터 채워짐
...
- **`game_teams.members` 신규 필드** (`damage_to_player`, `damage_from_player`, `play_time`): 크롤러에 추가됨, 재수집 이후 데이터부터 채워짐
